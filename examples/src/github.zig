...
pub fn main() !void {
    ...
    router.get("/api/v1/oauth/github", handleLogin, .{});
    router.get("/api/v1/oauth/github/callback", handleCallback, .{});

    std.log.info("Running GitHub example project on port 3000", .{});
    try server.listen();
}
